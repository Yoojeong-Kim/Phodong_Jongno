...
export async function POST(req:Request){
 ...
 try{
  ...
  const existing=await env.STORY_IMAGES.head(key);
  if(existing){
  ...
  slotAcquired=true;
  const prompt=`장면 ${page+1}/5: ${target.image_prompt}\n\n첨부 이미지는 절대적인 시각 기준이다. 같은 제작사, 같은 3D 렌더러, 같은 캐릭터 모델 시트로 만든 한 권의 동화처럼 보여야 한다. 기준 이미지의 밝고 화사한 복숭아·블러시 핑크·크림 팔레트, 깨끗한 흰색 기준점, 부드러운 아침 햇빛, 포근한 니트와 털의 미세한 촉감, 둥글고 친근한 얼굴 비율, 맑고 반짝이는 검은 눈, 생기 있는 분홍 볼, 고급 장편 애니메이션 수준의 정교한 3D 렌더링을 그대로 유지한다. 전체 화면은 밝고 따뜻하며 사랑스럽고 안전해야 한다.\n\n캐릭터·물건 잠금: 포동이의 얼굴, 귀, 눈, 주둥이, 체형, 털색, 목 스카프를 기준 이미지와 동일하게 유지한다. image_prompt에 적힌 모든 주인공의 얼굴형, 눈, 코, 머리 길이와 모양, 피부색, 체형, 옷의 색과 무늬를 1페이지와 완전히 동일하게 유지한다. 모든 주인공을 비슷한 크기로 또렷하게 보여 주고 누구도 가리거나 잘라내지 않는다. 다음 모든 물건 '${story.object_name}'도 빠짐없이 화면에 넣고, 각각의 색·형태·재질·크기를 유지하며 동등한 시각적 비중을 준다. 페이지마다 바꾸는 것은 이야기 내용에 따른 장소, 자세, 표정, 행동, 카메라 구도뿐이다. 새 캐릭터로 재해석하거나 나이·성별·머리·옷을 바꾸지 않는다.\n\n금지: 주인공이나 물건 누락, 한 명만 크게 강조, 탁한 갈색 또는 회색 필터, 어두운 노출, 공포 분위기, 날카로운 그림자, 무서운 표정, 움푹한 눈, 기괴한 손, 낡거나 더러운 재질, 저채도, 빈티지 세피아, 값싼 플라스틱, 다른 화풍, 실사 인물, 텍스트, 자막, 로고, 워터마크.`;
