...
export async function PUT(req:Request){
 try{
  ...
  if(drawings.some(v=>!v.id||!Number.isInteger(v.page)||v.page<0||v.page>4||!/^#[0-9a-f]{6}$/i.test(v.color)||![4,7,10,12,18,28].includes(v.width)||v.points.length<1||v.points.some((p:any)=>!Number.isFinite(p.x)||p.x<0||p.x>100||!Number.isFinite(p.y)||p.y<0||p.y>100)))return Response.json({error:"그림을 저장하지 못했어."},{status:400});
