...
export type CharacterInfo={name:string;appearance:string};
export type StoryRecord={id:string;child_name:string;genre:string;object_name:string;title:string;summary:string;pages:StoryPage[];characters?:CharacterInfo[];stickers?:unknown[];drawings?:unknown[];status:string;created_at:number};

export async function ensureStoryTables(){
 const db=env.DB;
 await db.batch([
  db.prepare(`CREATE TABLE IF NOT EXISTS stories (id TEXT PRIMARY KEY, child_name TEXT NOT NULL, genre TEXT NOT NULL, object_name TEXT NOT NULL, title TEXT NOT NULL, summary TEXT NOT NULL, pages_json TEXT NOT NULL, characters_json TEXT DEFAULT '[]', status TEXT NOT NULL DEFAULT 'generating', created_at INTEGER NOT NULL)`),
...
export function rowToStory(row:any):StoryRecord{
 ...
 return {...row,pages:JSON.parse(row.pages_json),characters:chars,...decoration};
}
